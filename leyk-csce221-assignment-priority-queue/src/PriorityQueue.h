...
private:
    ...
    void downheap(size_type index) {
        ...
        size_t max = index;
        size_t lhs = left_child(index);
        size_t rhs = right_child(index);
        if(lhs < size() && comp(c[max], c[lhs])){
            max = lhs;
        } 
        if(rhs < size() && comp(c[max], c[rhs])){
            max = rhs;
        }
        if(max != index){
            std::swap(c[max], c[index]);
            downheap(max);
        }
    }
...
public:
     ...
     */
    const_reference top() const {return const_reference(c[0]);}
